...
wind.title("Ping Pong by Mohamed_Teljaoui")                                 # set the title of the window
...
madrab1.shapesize(stretch_wid=5, stretch_len=1)                  #strectches the shape to meet  the size
madrab1.penup()                      #stops the objecct from drawing lines
...
wind.onkeypress(madrab1_up, "s")     #when pressing w the function madrab1_up is invoked
...
while True:
    ...
    ball.sety(ball.ycor() + ball.dy)   #ball starts at 0 and everytime loops run --->+0.3 xaxis

    #border check  
    if ball.ycor() >290:   #if  ball is at top border
        ball.sety(290)  # set y coordinate +290
        ball.dy *= -1   #reverse direction, making +0.3 --->2.5
    ...
    if ball.xcor() >390:      #if  ball is at right border
        ball.goto(0, 0)      #return bell to center
